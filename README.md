...
> Catatan: karena semua gambar produk memakai layanan placeholder (`placehold.co`) agar proyek tetap ringan tanpa aset biner, folder `images/` tidak diperlukan pada versi ini. Jika produk sungguhan tersedia, cukup buat folder `images/` dan ganti nilai `image` pada `PRODUCTS` di `app.js`.
